...
class DbdApp(ctk.CTk):
    def __init__(self):
        ...
        self.owned_killers = {obj["name"]: ctk.BooleanVar(value=self.settings.get("killers", {}).get(obj["name"], False)) 
                              for obj in self.data["killers"].values()}
    ...
    def load_data(self):
        ...
        return {
            "survivors": [s["name"] for s in s_raw if isinstance(s, dict) and "name" in s],
            "killers": k_raw,
            "perks": {
                "survivor": [item["name"] for item in p_surv.values() if isinstance(item, dict) and "name" in item],
                "killer": [item["name"] for item in p_kill.values() if isinstance(item, dict) and "name" in item]
            },
            "offerings": {
                "survivor": [item["name"] for item in off_surv.values() if isinstance(item, dict) and "name" in item],
                "killer": [item["name"] for item in off_kill.values() if isinstance(item, dict) and "name" in item]
            },
            "items": [
            ...
            "addons_survivor": [
                {"name": val["name"], "item_type": val.get("item_type")}
                for val in addons_surv.values() if isinstance(val, dict) and "name" in val
            ],
            "addons_killer": addons_kill 
        }
    ...
    def setup_generator(self):
        ...
        # --- RÁMEC PRO VOLBY NA JEDNOM ŘÁDKU ---
        self.options_frame = ctk.CTkFrame(self.tab_gen, fg_color="transparent")
        self.options_frame.pack(pady=15)
        
        # Všechny checkboxy přidáváme s parametrem side="left", aby se řadily vedle sebe
        ...
        # Výchozí stav (skryto), zobrazí se až se zaklikne Item
        # --------------------------------------
    ...
    def toggle_addon_visibility(self):
        if self.role_var.get() == "Survivor":
            if self.check_item.get():
                self.check_addons.pack(side="left", padx=10) # Přidá se vedle na řádek
            else:
                self.check_addons.pack_forget()
                self.check_addons.deselect()

    def update_ui(self, role):
        # Nejdřív oba schováme
        self.check_item.pack_forget()
        self.check_addons.pack_forget()
        
        if role == "Killer": 
            # U killera ukazujeme jen Addony vedle Offeringu
            self.check_addons.pack(side="left", padx=10)
        else: 
            # U survivora ukážeme Item a podle něj i případně Addony
            self.check_item.pack(side="left", padx=10)
    ...
    def generate(self):
        ...
        else: choices_objs = [obj for obj in self.data["killers"].values() if self.owned_killers.get(obj["name"], ctk.BooleanVar()).get()]
        ...
        if self.check_item.get() or self.check_addons.get():
            if role == "Survivor":
                ...
                killer_item_id = next((obj["item"] for obj in self.data["killers"].values() if obj["name"] == chosen_char_name), None)
                if killer_item_id:
                    addons_pool = [a['name'] for a in self.data["addons_killer"].values() if killer_item_id in a.get("parents", [])]
                    item_text = f"\nAddony: {', '.join(random.sample(addons_pool, min(2, len(addons_pool))))}" if addons_pool else "\nAddony: Žádné"
